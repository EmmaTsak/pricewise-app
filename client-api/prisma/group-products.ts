...
import { getProductMatchData } from "../src/utils/normalizeProduct";
import { calculateProductMatchPercentage } from "../src/utils/stringSimilarity";
...
async function main() {
  ...
  for (const product of products) {
    ...
    for (const group of groups) {
      /*
        Only compare inside the same PriceWise category.
      */
      ...
      */
      ...
      /*
        A group cannot contain two products from the same supermarket.
      */
      ...
      /*
        Brand rule for packaged products.

        If both products have brands and the brands are different,
        do not group them.
        ...
        These are similar products, but not the same exact product.
      */
    ...
    if (bestGroup && bestScore >= MATCH_THRESHOLD) {
      ...
      /*
        Helpful debug:
        show products that were close, but not close enough.
      */
...
for (const [groupIndex, group] of groups.entries()) {
  /*
    normalizedName must be unique inside the same category.

    We include groupIndex at the end so two similar groups
    do not crash the database.

    This does NOT affect the user-facing name.
    It is only an internal database key.
  */
...
main()
  .catch((error) => {
    console.error("Product grouping failed:", error);
    process.exit(1);
  })
  .finally(async () => {
